...
| Intake ID-gate + verify/adopt: intake sync requires IDs per sent slot, hintless read-back, adopt observed IDs, no phantom IDs | Real tickets (HTL-TVR-05301-11: response lost; HTL-TVR-05303: empty-ID pseudo-success accepted) | Active 2026-09-06 | `server.js` intake sync + `tests/intake-evidence-verification.test.js` 27/27 |
| Intake vacuous-verify guard + thumbnail-URL ID backfill (server record only) | Byte-less retries must not self-verify; old URL-only records recoverable without re-upload | Active 2026-09-06 | `server.js` + intake suite 36/36 |
| Emergency verify fix: resolved schoolFolder at call sites + UDISE-containment rule + adoption gated on folder match | Missing spec field made every read-back fail (live regression) | Active 2026-09-06 | `server.js` + call-site suite 11/11 |
